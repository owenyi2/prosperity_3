...
class SquidInk(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        order_depth: OrderDepth = state.order_depths[self.symbol]
        self.compute_midprice(state)
        fair_value = int(round(self.midprice))
        
        self.market_make(position, fair_value)

    def market_make(self: Strategy, position: int, fair_value: int) -> None:
        cpos = position
    
        our_bid = fair_value - self.spread 
        if position == -self.position_limit:
            our_bid = fair_value - self.spread // 2
        ...
        our_ask = fair_value + self.spread
        if position == self.position_limit:
            our_ask = fair_value + self.spread // 2
...
class Trader:
    def __init__(self) -> None:
        init_dict: dict[Symbol, Tuple[Type[Strategy], int, dict[str, Any]]] = {
                ...
                "SQUID_INK": (SquidInk, 50, {"spread": 2}),
