...
class VolcanicRockVoucher(ForecastStrategy):
    ...
    def reach_position_aggressive(self, position: int, desired_position: int, order_depth: OrderDepth, default_price: int) -> None:
        change = abs(desired_position - position)
        ...
        if desired_position > position:
            ...
            if len(sell_orders) > 0:
                ...
                vol = min(change, -sell_orders[best_ask])
                print(best_ask, vol)
                self.buy(best_ask, vol) 
            if change > vol: 
                self.buy(default_price + cross, change - vol)
        if desired_position < position:
            buy_orders = order_depth.buy_orders
            if len(buy_orders) < 0:
                best_bid = max(buy_orders)
                vol = min(change, buy_orders[best_bid])
                self.sell(best_bid, vol)
            if change > vol:
                self.sell(default_price - cross, change - vol)
    ...
    def act(self, state: TradingState) -> None:
        ...
        z_score = (self.option_chain.iv[self.i] - self.mean_iv) / self.std_iv

        if z_score > self.threshold:
            desired_position = -self.position_limit
        elif z_score < -self.threshold:
            desired_position = self.position_limit
        else:
            desired_position = 0
        
        mid_price = int(round(self.option_chain.midprice[self.i]))
        self.reach_position_aggressive(position, desired_position, order_depth, mid_price)
        print("iv", self.option_chain.iv[self.i])
